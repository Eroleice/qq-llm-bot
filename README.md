...
`config.toml`、`provider.json` 和 `.env` 已在 `.gitignore` 中排除，可放心填写本地群号、token 和 API key。
...
3. 如果要启用 LLM，把 provider 信息写入根目录 `provider.json`，把 `.env` 里的 key 环境变量改成实际 key，并在 `config.toml` 的 `[llm.router]` 里用 `provider:model` 选择模型。
...
1. 在 `provider.json` 和 `config.toml` 中修改（可参考 [config.example.toml](config.example.toml)）：
...
```json
{
  "providers": [
    {
      "id": "pro",
      "url": "https://你的 API 地址/v1",
      "type": "openai",
      "key_env": "OPENAI_API_KEY",
      "model": ["gpt-5.5", "gpt-5.4-mini", "你的生图模型名"]
    }
  ]
}
```
...
image_generation_model = "pro:你的生图模型名"

[image_generation]
enabled = false
```

`provider.json` 里的 `type = "openai"` / `"openai-compatible"` 会走 OpenAI-compatible
Chat Completions / Responses；`type = "gemini"` 会走 Gemini native
`models/{model}:generateContent`。模型选择统一使用 `provider_id:model`，例如
`pro:gpt-5.5`。
...
视觉模型在 `[llm.router]` 里配置：`simple_vision_model` 用于日常读图，
`detailed_vision_model` 用于直接问图、复杂读图和失败重试。
