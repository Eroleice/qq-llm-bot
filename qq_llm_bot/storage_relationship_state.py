...
from qq_llm_bot.storage_records import _clamp_score, _dashboard_user_id
...
def get_relationship(storage: Any, group_id: str, user_id: str) -> RelationshipState:
    ...
    if row is None:
        return RelationshipState(group_id=str(group_id), user_id=subject)
...
def apply_relationship_delta(
    ...
    if not subject:
        return RelationshipState(group_id=str(group_id), user_id=str(user_id))
    current = get_relationship(storage, group_id, subject)
